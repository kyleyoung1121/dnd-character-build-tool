...
export function getResolutionOptions(conflict: Conflict): ConflictResolution {
	...
	const recommendedAction = automaticSources.length > 0 && userSelectableSources.length > 0
...
export function resolveConflictByReverting(conflict: Conflict, scopeId: string): boolean {
	...
	const stillConflicted = newResult.conflicts.some(c => 
		c.type === conflict.type && 
		c.value === conflict.value
	);
...
function getExistingValues(character: Character, type: ConflictType): string[] {
	switch (type) {
		case 'skill': return character.skills || [];
		case 'proficiency': return character.proficiencies || [];
		case 'language': return character.languages || [];
		case 'feature': return character.features || [];
		default: return [];
	}
}
...
function isUserSelectableSource(source: string): boolean {
	...
	return source.includes('.proficiencies') || 
